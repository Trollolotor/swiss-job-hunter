...
const OPERATIONS = ["keyword_extraction","job_screening","company_summary","translation","cv_tailoring","cover_letter"];
...
export default function SettingsView({api, onProfilesChanged}) {
  ...
  const load=async()=>{
    ...
    setProfiles(p); setSelected(s=>p.find(x=>x.id===s?.id)||p[0]||null);
  ...
  const newProfile=()=>setSelected({id:"new-"+Date.now(),slug:"",name:"",keywords:[],cv_text:"",active:true});
  ...
  const saveLlm=async()=>{
    ...
    if(!r.ok) throw new Error(await r.text());
    setLlm(await r.json()); setMessage("AI routing saved");
  };
  ...
  const savePriority=async()=>{
    ...
    if(!r.ok) throw new Error(await r.text());
    setPriority(await r.json()); setMessage("Priority settings saved");
  };

  if(!llm||!priority) return <div style={{padding:30,color:"#5a7a68"}}>Loading settings…</div>;
  return <div style={{flex:1,overflowY:"auto",padding:20,display:"grid",gridTemplateColumns:"minmax(320px,1fr) minmax(440px,1.4fr)",gap:16,alignItems:"start"}}>
    <section style={box}>
      <div style={{fontWeight:800,marginBottom:12}}>ROLES & CV</div>
      <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:12}}>
        {profiles.map(p=><button key={p.id} style={{...button,background:selected?.id===p.id?"#2e7d5225":"transparent"}} onClick={()=>setSelected(p)}>{p.name}</button>)}
      ...
      {selected&&<div style={{display:"flex",flexDirection:"column",gap:9}}>
        ...
        <label>Search keywords
          <textarea style={{...input,minHeight:80}} value={(selected.keywords||[]).join("\n")} onChange={e=>setSelected({...selected,keywords:e.target.value.split("\n").map(x=>x.trim()).filter(Boolean)})}/>
        </label>
        <label>CV text
          <textarea style={{...input,minHeight:300}} value={selected.cv_text||""} onChange={e=>setSelected({...selected,cv_text:e.target.value})}/>
        </label>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <section style={box}>
        <div style={{fontWeight:800,marginBottom:4}}>OPENROUTER ROUTING</div>
        <div style={{fontSize:10,color:llm.openrouter_configured?"#2e7d52":"#c0392b",marginBottom:12}}>API key: {llm.openrouter_configured?"configured":"missing in environment"}</div>
        <label>Default model<input style={input} value={llm.default_model} onChange={e=>setLlm({...llm,default_model:e.target.value})}/></label>
        <label>Global fallbacks (one per line)<textarea style={{...input,minHeight:55}} value={(llm.global_fallbacks||[]).join("\n")} onChange={e=>setLlm({...llm,global_fallbacks:e.target.value.split("\n").map(x=>x.trim()).filter(Boolean)})}/></label>
        ...
        <div style={{display:"flex",gap:8,marginTop:12}}>
          {["timeout","retries","concurrency"].map(k=><label key={k} style={{fontSize:10}}>{k}<input type="number" style={{...input,width:90}} value={llm[k]} onChange={e=>setLlm({...llm,[k]:Number(e.target.value)})}/></label>)}
        </div>
      ...
      <section style={box}>
        ...
        <div style={{fontSize:10,color:"#5a7a68",margin:"8px 0"}}>Freshness buckets: {priority.buckets.map(b=>(b.max_hours/24)+"d="+b.score).join(" · ")}</div>
